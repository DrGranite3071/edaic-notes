...
\begin{itemize}
  ...
  \item \textbf{Depth matters}: anticholinesterases are unreliable in very deep block; reversal is performed once there is evidence of recovery (e.g.\ return of twitches on TOF).
\end{itemize}
...
\newpage
\section*{Appendix --- Additions from other references (clearly marked as non-Peck)}

\subsection*{A1) Practical monitoring sites and muscle-group differences (Non-Peck)}

\textbf{Monitoring sites (common clinical practice)}
\begin{itemize}
  \item \textbf{Ulnar nerve $\rightarrow$ adductor pollicis} (thumb adduction). \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item \textbf{Facial nerve $\rightarrow$ orbicularis oculi} (peri-orbital contraction). \textit{(Source: Morgan \& Mikhail, 5th ed.)}
\end{itemize}

\textbf{Muscle groups differ in sensitivity and recovery}
\begin{itemize}
  \item The \textbf{diaphragm} tends to recover earlier than the \textbf{adductor pollicis}. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item The \textbf{rectus abdominis} tends to recover earlier than the \textbf{adductor pollicis}. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item The \textbf{laryngeal adductors} tend to recover earlier than the \textbf{adductor pollicis}. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item The \textbf{orbicularis oculi} tends to recover earlier than the \textbf{adductor pollicis}. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item Monitoring at the thumb can therefore \textbf{overestimate} the degree of block at the diaphragm/upper airway muscles during recovery. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item Peripheral nerve stimulation is a useful guide but does not replace direct assessment of the muscle groups relevant to the clinical goal. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
\end{itemize}

\textbf{Situations where monitoring can mislead}
\begin{itemize}
  \item Atrophied muscle may appear relatively resistant due to receptor proliferation. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item Limbs affected by hemiplegia/nerve injury may appear relatively resistant due to receptor proliferation. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item Using such an extremity for monitoring can lead to \textbf{overdosing}. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
\end{itemize}

\subsection*{A2) Quantitative neuromuscular monitoring methods (Non-Peck)}
\begin{itemize}
  \item Tactile/visual assessment is convenient but \textbf{inaccurate} for small residual blocks. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item \textbf{Mechanomyography (MMG)} measures force of contraction against a preload. \textit{(Source: Fundamentals of Anaesthesia, 4th ed.)}
  \item MMG is accurate but awkward to set up and maintain. \textit{(Source: Fundamentals of Anaesthesia, 4th ed.)}
  \item \textbf{Acceleromyography (AMG)} measures acceleration of a moving digit (typically the thumb) with a piezoelectric sensor. \textit{(Source: Fundamentals of Anaesthesia, 4th ed.)}
  \item AMG is more objective than tactile assessment but is sensitive to joint position. \textit{(Source: Fundamentals of Anaesthesia, 4th ed.)}
  \item \textbf{Electromyography (EMG)} records evoked compound muscle action potentials. \textit{(Source: Fundamentals of Anaesthesia, 4th ed.)}
  \item EMG avoids joint/transducer mechanics but electrode geometry/position can alter readings. \textit{(Source: Fundamentals of Anaesthesia, 4th ed.)}
\end{itemize}

\subsection*{A3) Reversal and extubation thresholds emphasised elsewhere (Non-Peck)}
\begin{itemize}
  \item One referenced threshold for adequacy of reversal/extubation is \textbf{TOFR $>$ 0.90}. \textit{(Source: Cross \& Plunkett, 2008)}
  \item One referenced approach suggests \textbf{$\geq$3 twitches} should be present before administering an anticholinesterase reversal agent. \textit{(Source: Cross \& Plunkett, 2008)}
  \item Clinical indicators of adequate recovery include \textbf{sustained head lift ($\geq$5 s)}. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item Clinical indicators of adequate recovery include the ability to generate an inspiratory pressure of \textbf{at least $-25$ cmH$_2$O}. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item Clinical indicators of adequate recovery include a \textbf{forceful hand grip}. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item Local muscle hypothermia can reduce twitch tension (reported as about \textbf{6\% per $^\circ$C}). \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item Reduced twitch tension with hypothermia can confound monitoring. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
\end{itemize}

\subsection*{A4) Additional suxamethonium points in other texts (Non-Peck)}
\begin{itemize}
  \item Fasciculations are commonly described with suxamethonium. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
  \item Repeated dosing or infusion may progress from \textbf{Phase I} to \textbf{Phase II} features. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
  \item Some sources describe an increase in \textbf{intracranial pressure (ICP)} with suxamethonium. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
  \item One text describes increased intragastric pressure with a \textbf{decrease} in lower oesophageal sphincter pressure. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
  \item This conflicts with the Peck-based statement earlier in this canvas that lower oesophageal sphincter tone increases simultaneously such that reflux risk is not increased. \textit{(Source: Primary FRCA in a Box, 2nd ed.; Peck \& Harris 5th ed., Ch 12)}
  \item Another source notes that although suxamethonium transiently increases IOP, there is \textbf{no evidence} of worsened outcomes in open-globe injuries. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item The same source notes pretreatment with an NDMR does not reliably prevent the IOP rise. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item Transient masseter tone increase can occur after suxamethonium. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
  \item Marked masseter rigidity preventing laryngoscopy is abnormal and can be a warning sign of malignant hyperthermia. \textit{(Source: Morgan \& Mikhail, 5th ed.)}
\end{itemize}

\subsection*{A5) Receptor upregulation timing after denervation/burns (Non-Peck)}
\begin{itemize}
  \item One text emphasises that receptor upregulation takes time after denervation-type injury. \textit{(Source: Fundamentals of Anaesthesia, 4th ed.)}
  \item The same text suggests suxamethonium is best avoided after \textbf{48--72 hours} in these settings. \textit{(Source: Fundamentals of Anaesthesia, 4th ed.)}
  \item This differs from the Peck-based timing in this canvas for burns (risk from $\sim$24 hours). \textit{(Source: Fundamentals of Anaesthesia, 4th ed.; Peck \& Harris 5th ed., Ch 12)}
  \item Treat this as a potential \textbf{exam-trap discrepancy} across references. \textit{(Source: Fundamentals of Anaesthesia, 4th ed.; Peck \& Harris 5th ed., Ch 12)}
\end{itemize}

\subsection*{A6) Additional sugammadex points from another revision text (Non-Peck)}
\begin{itemize}
  \item One summary text quotes typical reversal times by depth of block. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
  \item With \textbf{2 mg\,kg$^{-1}$} when T2 reappears, reversal time is quoted as $\sim$\textbf{2 minutes}. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
  \item With \textbf{4 mg\,kg$^{-1}$} for deep block (1--2 PTC), reversal time is quoted as $\sim$\textbf{3 minutes}. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
  \item With \textbf{16 mg\,kg$^{-1}$} for rescue reversal, reversal time is quoted as $\sim$\textbf{1.5 minutes}. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
  \item A delay of \textbf{24 hours} is suggested before using rocuronium or vecuronium again after rescue reversal. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
  \item Bitter/metallic taste is listed as a possible adverse effect. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
  \item Bronchospasm is listed as a possible adverse effect. \textit{(Source: Primary FRCA in a Box, 2nd ed.)}
\end{itemize}
